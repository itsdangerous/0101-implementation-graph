...
class DirectedGraphTest {
	...
	private boolean isConnected(int from, int to) {
		return g.adjList[from].stream().anyMatch(node -> node.nodeNumber == to);
	}
	...
	private int findWeight(int from, int to) {
		return g.adjList[from].stream().filter(node -> node.nodeNumber == to)
				.findAny()
